...
- **Sign in with Apple wired in Clerk (2026-09-08):** App ID `app.spritzofficial` gained the Sign In with Apple capability; Services ID `app.spritzofficial.signin` (domain `clerk.spritzofficial.app`, return URL `https://clerk.spritzofficial.app/v1/oauth_callback`); a Sign in with Apple key; email sources `clkmail.spritzofficial.app` and Clerk's bounce address. Credentials entered in Clerk's Apple connection (custom credentials). Closes the slice 2 "Apple enabled in Clerk" item and Guideline 4.8 for submission. Device test pending.
