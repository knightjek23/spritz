...
## Post-release (2026-09-23)

- **"This app was removed from sale" the day after release.** Root cause: app-level **App Availability** on Pricing and Availability was never set up (the 175-territory selection during slice 7 was on the subscription products, not the app). A released app with zero territories shows as removed from sale. Fix: Set Up Availability → All Countries or Regions → Confirm; all 175 went "Processing to Available" (up to 24 h). Price schedule was already $0.00 everywhere. No resubmission, no build. Three things that pull a live app: Remove App From Sale, an unaccepted Paid Apps agreement update, lapsed tax/banking.
- **First post-release web changes (`ddb6789`, no build):** House rows in the search typeahead and /search (`lib/search-houses.ts`: in-memory index of `list_catalog_houses`, every query word a prefix of a house-name word, filler words ignored, aliases merged, max 2, link to /house/[slug]); shell-only CSS under `.native-app` for no long-press link preview / text-selection loupe / tap flash on links and buttons; Tailwind `future.hoverOnlyWhenSupported` so `hover:` no longer sticks after a tap on touch devices.
